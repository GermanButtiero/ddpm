...
class Diffusion:
    ...
    def get_betas(self, schedule='linear'):
        ...
        if schedule == 'cosine':
            return torch.tensor([self.beta_start + 0.5 * (self.beta_end - self.beta_start) * (1 + torch.cos(t / self.T * 3.1415926535)) for t in range(self.T)])
        else :
    ...
    def p_mean_std(self, model, x_t, t):
        ...
        mean = (x_t - alpha * predicted_noise) / alpha_bar # HINT: calculate the mean of the distribution p(x_{t-1} | x_t). See Eq. 11 in the ddpm paper at page 4 
    ...
    def p_sample(self, model, x_t, t):
        ...
        x_t_prev = mean + noise # Calculate x_{t-1}, see line 4 of the Algorithm 2 (Sampling) at page 4 of the ddpm paper.
